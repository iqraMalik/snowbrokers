...
$check_login = $this->session->userdata('userId');

if( isset($check_login) && $check_login > 0 )
{
    echo "Welcome User...";
    $check_login = $this->session->unset_userdata('userEmail');
    $check_login = $this->session->unset_userdata('userId');
    $check_login = $this->session->unset_userdata('user_logged_in');
}
else
{

$attributes = array('class' => 'form-horizontal', 'id' => 'loginForm', 'onSubmit' => 'return chkFrm();');
//form validation
echo validation_errors();
//echo form_open('admin/pages/add', $attributes);
echo form_open_multipart('login/Login', $attributes);
?>
<table>
    <tr>
        <td>User Email : </td>
