    ...
    */
        ...
        public function getAllCategory($search_option,$get_search) 
	{
		if(($search_option!="") && ($get_search!=""))
		{
			...
			if($search_option=="all")
			{
				$this->db->where('size_type like "%'.$get_search.'%"');
			}
	...
	public function getCategory($search_option,$get_search,$limit_start=0,$limit=0) 
	{
		if(($search_option!="") && ($get_search!=""))
		{
			...
			$this->db->order_by("id", "DESC");
			$q = $this->db->get('product_size',$limit,$limit_start);
	...
	public function CategoryType()
	{
		...
		if($this->input->post('category_type')=="")
		{
			$main_id=0;
	          ...
	          $query=$this->input->post('size');
		  $this->db->where('size_type',$query);
		  $query=$this->db->get('product_size');
		  if($query->num_rows()>0)
		  {
		$this->session->set_userdata('error_msg','<div class="alert alert-error"><a class="close" data-dismiss="alert">x</a><strong>Duplicate entery. Please try again</strong></div>');
		  redirect('product_size/index');
			...
			'size_type' => $this->input->post('size'),
			'product_type' => $this->input->post('product_type'),
			'product_category' => $this->input->post('product_category'),
			'status' => $this->input->post('status'),
		);
	  
		$insert = $this->db->insert('product_size', $new_member_insert_data);
...
//			$this->session->set_userdata('error_msg','<div class="alert alert-error"><a class="close" data-dismiss="alert">x</a><strong>Failed to add article. Please try again</strong></div>');
        ...
        public function edit_category()
	{
		...
		$insert = $this->db->update('product_size',$new_member_insert_data);
		//echo $this->db->last_query();die;
        ...
        public function multiple_category_delete()
	{
		...
		if($query)
			...
			$this->session->set_userdata('error_msg','<div class="alert alert-error"><a class="close" data-dismiss="alert">x</a><strong>Failed to delete Product size. Please try again</strong></div>');
        ...
        public function multiple_category_active_inactive($get)
	{
		...
		if($allbox>0)
		{
			foreach($scripts as $key=>$value)
			{
				...
				$this->db->where('id', $value);
				$query = $this->db->update('product_size',$new_insert_data);
			}
		}
		else
		{
	...
	{
		$this->db->where('size_type', $user_name);
		$query = $this->db->get('product_size');
	...
	function get_paged_list($limit = 0, $offset = 0)
	{
		...
		$q = $this->db->get('product_size',$limit,$offset);
