...
class product_subsubcat extends CI_Controller {
    ...
    */	
	function __construct()
	{
		...
		$this->load->model('model_productsubsubcat');
		
		if(!$this->model_productsubsubcat->isLoggedIn()){
			redirect("login/index");
		}
		 $this->limit = $this->model_productsubsubcat->setting_pagination();
	...
	function index()
	{
		...
		if($this->input->post('search_mode')==='search')
		{
			...
			$config['total_rows'] = $this->model_productsubsubcat->count_all_cat($searchparam);
			
			$offset = 0;
			$uri_segment = $offset;
			$parents = $this->model_productsubsubcat->get_paged_list_cat($this->limit,$offset,$searchparam);
		...
		$config['base_url'] = base_url().'index.php/product_subsubcat/index/'.$searchparams.'/';
  		$config['per_page'] = $this->limit;
		$config['uri_segment'] = $uri_segment;
		$this->pagination->initialize($config);
		$data['pages'] = $this->pagination->create_links();
		$data['rows'] = $parents;
		$this->load->view('product_subsubcat/index',$data);
		
	}
	
	function addproductsubsubcat()
	{
		$this->load->view('product_subsubcat/addsubsubcat');
	}
	function editsubsubcat()
	{
		if($this->input->post('ListingUserid')!=0)
			$this->load->view('product_subsubcat/editsubsubcat');
		else
			redirect('product_subsubcat/index');
	}
	
	public function edit_subsubcat()
	{

		$insertd = $this->model_productsubsubcat->edit_subsubcat();
		if($insertd)
	...
	function insert_subsubcat()
	{
		
		
		$insertd = $this->model_productsubsubcat->insert_subsubcat();
		if($insertd)
	...
	public function delete_subsubcat()
	{
		...
		$insertd = $this->model_productsubsubcat->delete_subsubcat();
		if($insertd)
	...
	function get_All_subsubcat() {
		$data['rows'] = $this->model_productsubsubcat->getAllArticles();
	}
	public function multiple_option_subsubcat()
	{
		$data['success_msg'] 	= $this->session->userdata('success_msg');
		$data['error_msg'] 	= $this->session->userdata('error_msg');
		...
		{
			$insertd = $this->model_productsubsubcat->multiple_subsubcat_delete();
		}
		else if($this->input->post('select_my_option')==2)
		{
			$insertd = $this->model_productsubsubcat->multiple_subsubcat_active_inactive(1);
		}
		else if($this->input->post('select_my_option')==3)
		{
			$insertd = $this->model_productsubsubcat->multiple_subsubcat_active_inactive(0);
		}
