...
class product_color extends CI_Controller {
    ...
    */	
	function __construct()
	{
		...
		if(!$this->model_color->isLoggedIn()){
			redirect("login/index");
		}
		 $this->limit = $this->model_color->setting_pagination();
	...
	function index()
	{
		...
		if($this->input->post('search_mode')==='search')
		{
			...
			if($this->input->post('sort_field')!='')
			{
				...
				//set cookies
				delete_cookie('snowbrokers_colors_type');
				delete_cookie('snowbrokers_colors_field');
				$cookie = array(
				'name'   => 'snowbrokers_colors_type',
				'value'  => $sort_type,
			...
			if($this->input->post('get_search')!='')
			{
				...
				$searchtext = 'color_code='.urlencode($this->input->post('get_search'));
			...
			$config['total_rows'] = $this->model_color->count_all_cat($searchparam);
			
			$offset = 0;
			$uri_segment = $offset;
			$parents = $this->model_color->get_paged_list_cat($this->limit,$offset,$searchparam);
		...
		else if($last>3)
		{
			...
			$sort_type = get_cookie('snowbrokers_colors_type');
			$sort_field = get_cookie('snowbrokers_colors_field');
			$data['sort_type'] = $sort_type;
			$data['sort_field'] = $sort_field;
		...
		$config['base_url'] = base_url().'index.php/product_color/index/'.$searchparams.'/';
  		$config['per_page'] = $this->limit;
		$config['uri_segment'] = $uri_segment;
		$this->pagination->initialize($config);
		$data['pages'] = $this->pagination->create_links();
		$data['rows'] = $parents;
		$this->load->view('product_color/index',$data);
		
	}
	
	function addcolor()
	{
		$result['chkbox']=$this->model_color->chk();
		$result['product_type']=$this->model_color->product_type();
		$this->load->view('product_color/addcolor',$result);
	}
	function editcolor()
	{
		if($this->input->post('ListingUserid')!=0)
		{
			$result['chkbox']=$this->model_color->chk();
			$result['product_type']=$this->model_color->product_type();
			$this->load->view('product_color/editcolor',$result);
		}
		else
	...
	function choose_category(){
		...
		$data['product_type']=$this->model_color->choose_category($product_type);
		$this->load->view('product_color/category_ajax',$data);
	}
	
	public function edit_category()
	{

		$insertd = $this->model_color->edit_category();
		if($insertd)
	...
	function insert_color()
	{
		...
		$insertd = $this->model_color->insert_color();
		if($insertd)
			$this->session->set_userdata('success_msg', '<div class="alert alert-success"><a class="close" data-dismiss="alert">x</a><strong>Product color  added successfully</strong></div>');
		else
			$this->session->set_userdata('error_msg', '<div class="alert alert-error"><a class="close" data-dismiss="alert">x</a><strong>Failed to added Product color. Please try again</strong></div>');
	...
	public function delete_category()
	{
		...
		$insertd = $this->model_color->delete_category();
		if($insertd)
			$this->session->set_userdata('success_msg', '<div class="alert alert-success"><a class="close" data-dismiss="alert">x</a><strong>Brand deleted successfully</strong></div>');
		else
			$this->session->set_userdata('error_msg', '<div class="alert alert-error"><a class="close" data-dismiss="alert">x</a><strong>Failed to deleted Brand. Please try again</strong></div>');
	...
	function get_All_category() {
		$data['rows'] = $this->model_color->getAllArticles();
	}
	public function multiple_option_category()
	{
		...
		{
			$insertd = $this->model_color->multiple_category_delete();
		}
		else if($this->input->post('select_my_option')==2)
		{
			$insertd = $this->model_color->multiple_category_active_inactive(1);
		}
		else if($this->input->post('select_my_option')==3)
		{
			$insertd = $this->model_color->multiple_category_active_inactive(0);
		}
