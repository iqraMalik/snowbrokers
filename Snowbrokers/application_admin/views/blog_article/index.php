...
<script type="text/javascript">
	function go_search()
	{
		...
		var selected1 = $("#cat_type").val();
		//alert(selected);
		if ((selected == '') && (document.getElementById('get_search').value == "") && (selected1 != ''))
		{
	...
	function seaarch_cat() {
		...
		{
		   document.getElementById('category_input').value = document.getElementById('cat_type').value;
		   document.getElementById('search_form').submit();
		...
		<div id="main-stats">
			<div class="table-products">
				...
				<div class="row-fluid filter-block">
					...
					<div class="pull-right">
						<form name="search_form" id="search_form" method="post" action="">
							...
							?>
							<input type="hidden" id="sort_type" name="sort_type" value="<?php echo (isset($sort_type)?$sort_type:get_cookie('taskaroo_blogarticle_type'));?>" />
							<input type="hidden" id="sort_field" name="sort_field" value="<?php echo (isset($sort_field)?$sort_field:get_cookie('taskaroo_blogarticle_field'));?>" />
							...
							<div class="btn-group pull-left" style="padding-right: 10px;">
								...
								<button class="glow left large" href="javascript:void(0);" value="0" onclick="display_active_inactive(0);"><span style="<?php echo $style2;?>"> In-Active </span></button>
							      ...
							      <div class="ui-select">
								   ...
								   <select name="cat_type" id="cat_type" label="Blog Type" >
								      ...
								      {
									?>
								      <option value="<?php echo $data_type->id;?>" <?php if(isset($cat_type) && ($cat_type==$data_type->id))echo ' selected="selected"';?>><?php echo $data_type->blogname;?></option>     
								      <?php
								      }
								      ?>
								   </select>
				...
				<form name="multiple_form" id="multiple_form" method="post" action="<?php echo site_url('blog_article/multiple_option_blog'); ?>">
					...
					<input type="hidden" name="cat_type" id="category_input" value="<?php if(isset($cat_type)){echo $cat_type;}?>">
					<div class="row-fluid">
						<table class="table table-hover">
							<thead>
								<tr>
									<th class="span1"><input type="checkbox" name="check_all" id="check_all" style="margin: 0px;"></th>
									<th class="span5"><a id="title" href="javascript:void(0);" onclick="sortField('title');" style="color:#000;text-decoration: none;">Blog Title
										<?php
											if($sort_field=='title')
											{
												if($sort_type=="DESC")
									...
									<th class="span5"><a id="title" href="javascript:void(0);" onclick="sortField('blog_catid');" style="color:#000;text-decoration: none;">Blog Category Name
										<?php
											if($sort_field=='blog_catid')
											{
												if($sort_type=="DESC")
												{
													echo "<img src='".base_url()."images/sort_desc.png' />";
									...
									<th class="span3"><a id="title" href="javascript:void(0);" onclick="sortField('added_date');" style="color:#000;text-decoration: none;">Added Date
										<?php
											if($sort_field=='added_date')
											{
												if($sort_type=="DESC")
												{
													echo "<img src='".base_url()."images/sort_desc.png' />";
									...
									<th class="span3"><a id="title" href="javascript:void(0);" onclick="sortField('status');" style="color:#000;text-decoration: none;">Status
										<?php
								...
								<?php
									...
									?>
										...
										<tr class="first">
											...
											<td><?php //echo $dataone->blog_catid; ?>
								<?php
