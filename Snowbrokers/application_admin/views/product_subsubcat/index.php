...
<script type="text/javascript">
	...
	function go_reset()
	{
		location.href="<?php echo base_url();?>index.php/product_subsubcat/index";
	}
...
<div class="table-products">
	<div class="row-fluid head" style="border-bottom: 1px solid #C5C5C5; margin-bottom: 30px;">
		<div class="span12">
			<h4 class="management_name">Product subsubcategory Management</h4>
			<div class="pull-right" style="margin-bottom: 5px;"><a class="btn-flat new-product" href="<?php echo site_url('product_subsubcat/addproductsubsubcat'); ?>">+ Add New</a></div>
		</div>
	</div>
	<div class="row-fluid">
		<div id="main-stats">
			<div class="table-products">
				...
				<div class="row-fluid filter-block">
					...
					<div class="pull-left">
						<form name="search_form" id="search_form" method="post" action="">
							...
							?>
							<input type="hidden" id="sort_type" name="sort_type" value="<?php echo (isset($sort_type)?$sort_type:get_cookie('s_cat_type'));?>" />
							<input type="hidden" id="sort_field" name="sort_field" value="<?php echo (isset($sort_field)?$sort_field:get_cookie('s_cat_field'));?>" />
							...
							<?php
							   //    $TypeData = $this->model_productcategory->parentcat();
							      ...
							      <div class="ui-select" style="width:160px">
								   ...
								   <select name="cat_type" id="cat_type" label="Category Type">
								     ...
								     // foreach($TypeData as $data_type)
								      {
								      ?>
								     <!-- <option value="<?php echo $data_type->id;?>" <?php if(@$cat_type==$data_type->id)echo ' selected="selected"';?>><?php echo $data_type->name;?></option>     -->
								      <?php
								      }
								      ?>
								   </select>
							...
							<input type="text" class="search" name="get_search" placeholder="Search by Category Name" id="get_search" value="<?php if(isset($_REQUEST['get_search']) ){echo $_REQUEST['get_search'];}?>">
				...
				<form name="multiple_form" id="multiple_form" method="post" action="<?php echo site_url('product_subsubcat/multiple_option_subsubcat'); ?>">
						...
						<table class="table table-hover">
							<thead>
								<tr>
									...
									<th class="span3">
										<a id="main_id" href="javascript:void(0);" onclick="sortField('main_id');" style="color:#000;text-decoration: none;">
	
										Product Type
										<?php
												if($sort_field=='main_id')
												{
													if($sort_type=="DESC")
													{
														echo "<img src='".base_url()."images/sort_desc.png' />";
							...
							<tbody>
								<?php
											...
											<?php $subsubcat_id = $dataone->sub_cat_id;
											$scat = $this->model_productsubsubcat->get_subsubcat($subsubcat_id);
                                                                                        //print_r($scat);
											?>
											
											<td><?php echo $scat[0]['title'];?></td>
...
<script type="text/javascript">
	$(function () {
        $('.Edituser').click(function (e) {
			...
			$("#frmValidation").attr("action", "<?php echo site_url('product_subsubcat/editsubsubcat');?>");
	...
	$(function () {
        $('.Deleteuser').click(function (e) {
			...
			{
			    $('#ListingUserid').val($.trim(stripdata));
			    $("#frmValidation").attr("action", "<?php echo site_url('product_subsubcat/delete_subsubcat');?>");
			    $("#frmValidation").submit();
			    return true;
			}
		});
