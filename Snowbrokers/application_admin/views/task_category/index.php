...
<script type="text/javascript">
	...
	function go_reset()
	{
		location.href="<?php echo base_url();?>index.php/task_category/index";
	}
...
<div class="table-products">
	<div class="row-fluid head" style="border-bottom: 1px solid #C5C5C5; margin-bottom: 30px;">
		<div class="span12">
			<h4 class="management_name">Task Category Management</h4>
			<div class="pull-right" style="margin-bottom: 5px;"><a class="btn-flat new-product" href="<?php echo site_url('task_category/add_task_category'); ?>">+ Add New</a></div>
		</div>
	</div>
	<div class="row-fluid">
		<div id="main-stats">
						...
						<form name="search_form" id="search_form" method="post" action="">
							...
							<div class="ui-select">
								<select name="search_option" id="search_option">
									...
									<option value="title" <?php if(isset($search_option) && ($search_option=='title')){echo 'selected="selected"';}?>>Task Category Title</option>
								</select>
							</div>
		<input type="text" class="search" name="get_search" id="get_search" value = "<?php if(isset($search_text) && ($search_text!="")){echo $search_text;}?>">
				...
				<form name="multiple_form" id="multiple_form" method="post" action="<?php echo site_url('task_category/multiple_option_category'); ?>">
						...
						<table class="table table-hover">
							<thead>
								<tr>
									...
									<th class="span5">
										<a id="title" href="javascript:void(0);" onclick="sortField('title');" style="color:#000;text-decoration: none;">Task Category Title
										<?php
											if($sort_field=='title')
											{
												if($sort_type=="DESC")
							...
							<tbody>
								<?php
									if(count($task_category_list) > 0)
									{
										foreach ($task_category_list as $dataone):
									?>
										<!-- row -->
										<tr class="first">
											...
											<td>
											<img src="<?php echo GET_LOGO_PATH;?>task_cat_logo/thumbnail/<?php echo $dataone->category_logo; ?>" height="80" width="100">
											</td>
...
<script type="text/javascript">
	$(function () {
        $('.Edit_task_cat').click(function (e) {
			...
			$("#frmValidation").attr("action", "<?php echo site_url('task_category/edit_task_category');?>");
	...
	$(function () {
        $('.Delete_task_cat').click(function (e) {
			...
			var r=confirm("Are you sure to delete this task category?");
			
			if (r==true)
			{
			    $('#ListingTaskid').val($.trim(stripdata));
			    $("#frmValidation").attr("action", "<?php echo site_url('task_category/delete_task_category');?>");
			    $("#frmValidation").submit();
			    return true;
			}
		});
