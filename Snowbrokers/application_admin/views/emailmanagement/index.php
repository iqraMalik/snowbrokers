...
<script>
    function go_reset()
	{
		location.href="<?php echo base_url();?>index.php/emailmanagement/index";
	}
    function go_search()
    {
    ...
    <div class="row-fluid head" style="border-bottom: 1px solid #C5C5C5; margin-bottom: 30px;">
        <div class="span12" style="margin-left: -20px;">
	      ...
	      <div class="pull-right">
				 <a class="btn-flat new-product" href="<?php echo site_url('emailmanagement/addemail'); ?>">+ Add New</a>
			</div>
        </div>
    </div>
    <div class="row-fluid">
			...
			<form name="search_form" id="search_form" method="post" action="">
			    ...
			    <?php
							...
							?>
							<input type="hidden" id="sort_type" name="sort_type" value="<?php echo (isset($sort_type)?$sort_type:get_cookie('snowbrokers_article_type'));?>" />
							<input type="hidden" id="sort_field" name="sort_field" value="<?php echo (isset($sort_field)?$sort_field:get_cookie('snowbrokers_article_field'));?>" />
				    ...
				    <button class="glow left large" href="javascript:void(0);" value="0" onclick="display_active_inactive(0);"><span style="<?php echo $style2;?>"> In-Active </span></button>
                            ...
                            <div class="ui-select">
                                <select name="search_option" id="search_option">
                                  <option value="subject" <?php if(isset($search_option) && ($search_option=='subject')){echo 'selected';}?>>Subject</option>
		  ...
		  <form name="multiple_form" id="multiple_form" method="post" action="<?php echo site_url('emailmanagement/multiple_option_email'); ?>">
			    ...
			    ?>
					...
					<ul class="actions">
                              ...
                              <?php endforeach;
                                    ...
                                    <td colspan="6">
                                    	<div class="span12">
				                             <h4>There is no Email template at this time !! <a class="btn-flat new-product" href="<?php echo site_url('emailmanagement/addemail'); ?>">+ Add New</a> For Create New One </h4>
				                        </div>
                                    </td>
                                    </tr>
                              <?php } ?>
...
<script type="text/javascript">
	$(function () {
        $('.Edituser').click(function (e) {
			...
			$("#frmValidation").attr("action", "<?php echo site_url('emailmanagement/editemail');?>");
	...
	$(function () {
        $('.Deleteuser').click(function (e) {
			...
			{
			    $('#ListingUserid').val($.trim(stripdata));
			    $("#frmValidation").attr("action", "<?php echo site_url('emailmanagement/delete_email');?>");
			    $("#frmValidation").submit();
			    return true;
			}
		});
