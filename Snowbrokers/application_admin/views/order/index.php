...
<script type="text/javascript">
	...
	function go_reset()
	{
		location.href="<?php echo base_url();?>index.php/order/index";
	}
...
<div class="table-products">
	<div class="row-fluid head" style="border-bottom: 1px solid #C5C5C5; margin-bottom: 30px;">
		<div class="span12">
			<h4 class="management_name">Order Management</h4>
	<!--		<div class="pull-right" style="margin-bottom: 5px;"><a class="btn-flat new-product" href="<?php echo site_url('product/addproduct'); ?>">+ Add New</a></div>-->
		</div>
	</div>
	<div class="row-fluid">
		<div id="main-stats">
			<div class="table-products">
				...
				<div class="row-fluid filter-block">
					...
					<div class="pull-left">
						<form name="search_form" id="search_form" method="post" action="">
							...
							?>
							<input type="hidden" id="sort_type" name="sort_type" value="<?php echo (isset($sort_type)?$sort_type:get_cookie('snowbrokers_order_type'));?>" />
							<input type="hidden" id="sort_field" name="sort_field" value="<?php echo (isset($sort_field)?$sort_field:get_cookie('snowbrokers_order_field'));?>" />
							...
							<div class="ui-select" style="width:160px">
								...
								<select name="search_option" id="search_option">
									...
									<option value="order_transaction_id" <?php if(isset($_REQUEST['search_option']) && ($_REQUEST['search_option']=='order_transaction_id')){echo "selected";}?>>Order Id</option>
									<option value="f_name" <?php if(isset($_REQUEST['search_option']) && ($_REQUEST['search_option']=='f_name')){echo "selected";}?>>Order By</option>
									<option value="order_date" <?php if(isset($_REQUEST['search_option']) && ($_REQUEST['search_option']=='order_date')){echo "selected";}?>>Order Date</option>
									<option value="total_amount" <?php if(isset($_REQUEST['search_option']) && ($_REQUEST['search_option']=='total_amount')){echo "selected";}?>>Order Amount</option>
								</select>
							</div>
							<input type="text" class="search" name="get_search" placeholder="Enter Search Text" id="get_search" value="<?php if(isset($_REQUEST['get_search']) ){echo $_REQUEST['get_search'];}?>">
				...
				<form name="multiple_form" id="multiple_form" method="post" action="<?php echo site_url('order/multiple_option_order'); ?>">
						...
						<table class="table table-hover">
							<thead>
								<tr>
									...
									<th class="span3">
										<a id="order_transaction_id" href="javascript:void(0);" onclick="sortField('order_transaction_id');" style="color:#000;text-decoration: none;">
										order id
										<?php
												if($sort_field=='order_transaction_id')
												{
													if($sort_type=="DESC")
													{
									...
									<th class="span3">
										<a id="f_name" href="javascript:void(0);" onclick="sortField('f_name');" style="color:#000;text-decoration: none;">
										Order By
										<?php
												if($sort_field=='f_name')
												{
													if($sort_type=="DESC")
													{
														echo "<img src='".base_url()."images/sort_desc.png' />";
									...
									<th class="span3">
										<a id="order_date" href="javascript:void(0);" onclick="sortField('order_date');" style="color:#000;text-decoration: none;">
							...
							<tbody>
								...
								?>
											...
											<td><?php echo $dataone->order_transaction_id; ?></td>
										        <td> <?php echo $dataone->f_name; ?></td>
                                                                                        <td><?php echo $dataone->order_date; ?></td>
											<td><?php echo $dataone->total_amount;?></td>
											<td><?php echo ($dataone->status==1)?"Active":"Blocked"; ?></td>
											<td>
												<ul class="actions">
...
<script type="text/javascript">
	$(function () {
        $('.Edituser').click(function (e) {
			...
			$("#frmValidation").attr("action", "<?php echo site_url('order/vieworder');?>");
	...
	$(function () {
        $('.Deleteuser').click(function (e) {
			...
			{
			    $('#ListingUserid').val($.trim(stripdata));
			    $("#frmValidation").attr("action", "<?php echo site_url('order/delete_order');?>");
			    $("#frmValidation").submit();
			    return true;
			}
		});
