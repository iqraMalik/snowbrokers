...
<div class="table-products">
	<div class="row-fluid head" style="border-bottom: 1px solid #C5C5C5; margin-bottom: 30px;">
		<div class="span12">
			<h4 class="management_name">City Management</h4>
			<div class="pull-right" style="margin-bottom: 5px;"><a class="btn-flat new-product" href="<?php echo site_url('citymanagement/addcity'); ?>">+ Add New</a></div>
		</div>
	</div>
	<div class="row-fluid">
		<div id="main-stats">
...
<?php  $CountryData = $this->modelcity->Get_State();?>
						<select name="search_option" id="search_option">
							...
							?>
								<option value="<?php echo $dataone->id; ?>" <?php if(@$cat_type==$dataone->id)echo ' selected="selected"';?>><?php
							...
							<!--<div class="ui-select">
								<select name="search_option" id="search_option">
									...
									<option value="CityName" <?php if(isset($_REQUEST['search_option']) && ($_REQUEST['search_option']=='CityName')){echo "selected";}?>>City Name</option>
								</select>
					</div>-->
							<input type="text" class="search" name="get_search" id="get_search" placeholder="Enter City Name" value="<?php if(isset($_REQUEST['get_search']) ){echo $_REQUEST['get_search'];}?>">
				...
				<form name="multiple_form" id="multiple_form" method="post" action="<?php echo site_url('citymanagement/multiple_option_city'); ?>">
						...
						<table class="table table-hover">
							<thead>
								<tr>
									...
									<th class="span5"><a id="title" href="javascript:void(0);" onclick="sortField('CityName');" style="color:#000;text-decoration: none;">City Name
										<?php
							...
							<tbody>
								<?php
									...
									?>
										<!-- row -->
										<tr class="first">
											<td><input type="checkbox" name="scripts[]" class="checkbox_class" id="checkbox_<?php echo $dataone->city_ID;?>" style="margin: 0px;" value="<?php echo $dataone->city_ID;?>" onclick="go_check_all();"></td>
											<td><?php echo $dataone->CityName; ?></td>
											
											
											<td><?php echo ($dataone->status)?"Active":"Blocked"; ?></td>
											<td>
												<ul class="actions">
													<li><a class="Edituser" id="Edituser<?php echo $dataone->city_ID;?>" href="javascript:void(0);"><i class="table-edit"></i></a></li>
													<li class="last"><a class="Deleteuser" id="Edituser<?php echo $dataone->city_ID;?>" href="javascript:void(0);"><i class="table-delete-new"></i></a></li>
												</ul>
											</td>
										</tr>
										<?php
...
<form class="new_user_form" action="" id="frmValidation" method="post">
	<input type="hidden" name="ListingCityid" id="ListingCityid" value="0">
</form>
<script type="text/javascript">
	$(function () {
        $('.Edituser').click(function (e) {
			...
			$('#ListingCityid').val($.trim(stripdata));
			$("#frmValidation").attr("action", "<?php echo site_url('citymanagement/editcity');?>");
	...
	$(function () {
        $('.Deleteuser').click(function (e) {
			...
			{
			    $('#ListingCityid').val($.trim(stripdata));
			    $("#frmValidation").attr("action", "<?php echo site_url('citymanagement/delete_city');?>");
			    $("#frmValidation").submit();
			    return true;
			}
		});
