...
?>
    ...
    <div class="row-fluid login-wrapper">
		...
		<p id="msg">Please enter your email address so we can send you an email to reset your password.</p>
                <form  id="frmValidation" name="frm_login" class="signin-wrapper" action="<?php echo site_url('login/do_forgotpassword');?>" method="post">
                <input type="text" id="email" name="email" class="span12 validate[required]" placeholder="Email">
               <input type="submit" value="Submit" class="btn-glow primary" id="createUser">
		<a class="btn-flat new-product" href="<?php echo site_url('login/index'); ?>">Cancel</a>
                </form>
            </div>
        </div>
        ...
        $(function () {
            ...
            $('.login').click(function (e) {
            	...
            	if($.trim($('#email').val())=='' || $.trim($('#email').val())==0) {
            		$('#email').attr('placeholder','Email Please');
            		$('#email').css('border-color','red');
            		return false;
            	} else if($.trim($('#email').val())=='' || $.trim($('#email').val())==0) {
            		$('#email').attr('placeholder','<?php echo $this->lang->line("Password_Please");?>');
            		$('#email').css('border-color','red');
            		return false;
            	} else {
