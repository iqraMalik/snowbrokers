...
<script type="text/javascript">
	...
	function go_reset()
	{
		location.href="<?php echo base_url();?>index.php/plan/index";
	}
...
<div class="table-products">
	<div class="row-fluid head" style="border-bottom: 1px solid #C5C5C5; margin-bottom: 30px;">
		<div class="span12">
			<h4 class="management_name">Offer Management</h4>
			<div class="pull-right" style="margin-bottom: 5px;"><a class="btn-flat new-product" href="<?php echo site_url('offer/addoffer'); ?>">+ Add New</a></div>
		</div>
	</div>
	<div class="row-fluid">
		<div id="main-stats">
			<div class="table-products">
				...
				<div class="row-fluid filter-block">
					...
					<div class="pull-right">
						<form name="search_form" id="search_form" method="post" action="">
							...
							?>
							<input type="hidden" id="sort_type" name="sort_type" value="<?php echo (isset($sort_type)?$sort_type:get_cookie('sports_plan_type'));?>" />
							<input type="hidden" id="sort_field" name="sort_field" value="<?php echo (isset($sort_field)?$sort_field:get_cookie('sports_plan_field'));?>" />
							...
							<div class="ui-select">
								<select name="search_option" id="search_option">
									...
									<option value="planname" <?php if(isset($_REQUEST['search_option']) && ($_REQUEST['search_option']=='planname')){echo "selected";}?>>Plan Name</option>
								</select>
							</div>
				...
				<form name="multiple_form" id="multiple_form" method="post" action="<?php echo site_url('plan/multiple_option_state'); ?>">
						...
						<table class="table table-hover">
							<thead>
								<tr>
									...
									<th class="span5"><a id="planname" href="javascript:void(0);" onclick="sortField('planname');" style="color:#000;text-decoration: none;">Plan Name
										<?php
											if($sort_field=='planname')
											{
												if($sort_type=="DESC")
												{
									...
									<th class="span5"><a id="price" href="javascript:void(0);" onclick="sortField('planfor');" style="color:#000;text-decoration: none;">Plan for
										<?php
											if($sort_field=='planfor')
											{
												if($sort_type=="DESC")
												{
													echo "<img src='".base_url()."images/sort_desc.png' />";
							...
							<th class="span5"><a id="price" href="javascript:void(0);" onclick="sortField('plantype');" style="color:#000;text-decoration: none;">Plan Type
										<?php
											if($sort_field=='plantype')
											{
												if($sort_type=="DESC")
												{
													echo "<img src='".base_url()."images/sort_desc.png' />";
								...
								<?php
									...
									?>
										...
										<tr class="first">
											...
											<td><?php
											echo $dataone_fetch = $this->modeloffer->getplanname($dataone->planname);
											
											
											?></td>
											<td><?php if($dataone->planfor==1){echo "user";}elseif($dataone->planfor==2){echo "Instructor";}else{"company";} ?></td>
											
									<td><?php if($dataone->plantype==1){echo "free";}else{echo "payable";} ?></td>
...
<script type="text/javascript">
	$(function () {
        $('.Edituser').click(function (e) {
			...
			$("#frmValidation").attr("action", "<?php echo site_url('offer/editoffer');?>");
	...
	$(function () {
        $('.Deleteuser').click(function (e) {
			...
			var r=confirm("Are you sure to delete this plan?");
			
			if (r==true)
			{
			    $('#ListingUserid').val($.trim(stripdata));
			    $("#frmValidation").attr("action", "<?php echo site_url('offer/delete_offer');?>");
			    $("#frmValidation").submit();
			    return true;
			}
		});
