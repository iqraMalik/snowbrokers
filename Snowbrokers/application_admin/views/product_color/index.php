...
<script type="text/javascript">
	...
	function go_reset()
	{
		location.href="<?php echo base_url();?>index.php/product_color/index";
	}
...
<div class="table-products">
	<div class="row-fluid head" style="border-bottom: 1px solid #C5C5C5; margin-bottom: 30px;">
		<div class="span12">
			<h4 class="management_name">Color Management</h4>
			<div class="pull-right" style="margin-bottom: 5px;"><a class="btn-flat new-product" href="<?php echo site_url('product_color/addcolor'); ?>">+ Add New</a></div>
		</div>
	</div>
	<div class="row-fluid">
		<div id="main-stats">
			<div class="table-products">
				...
				<div class="row-fluid filter-block">
					...
					<div class="pull-left">
						<form name="search_form" id="search_form" method="post" action="">
							...
							?>
							<input type="hidden" id="sort_type" name="sort_type" value="<?php echo (isset($sort_type)?$sort_type:get_cookie('snowbrokers_colors_type'));?>" />
							<input type="hidden" id="sort_field" name="sort_field" value="<?php echo (isset($sort_field)?$sort_field:get_cookie('snowbrokers_colors_field'));?>" />
							...
							<?php
							       $TypeData = $this->model_color->parentcat();
							...
							<div class="ui-select">
								<select name="search_option" id="search_option">
									...
									<option value="color_code" selected="selected">Color code</option>
									<!--<option value="title" <?php if(isset($_REQUEST['search_option']) && ($_REQUEST['search_option']=='color_code')){echo "selected";}?>>Title</option>-->
								</select>
							</div>
							
							<input type="text" class="search" name="get_search" placeholder="Search by color code" id="get_search" value="<?php if(isset($_REQUEST['get_search']) ){echo $_REQUEST['get_search'];}?>">
				...
				<form name="multiple_form" id="multiple_form" method="post" action="<?php echo site_url('product_color/multiple_option_category'); ?>">
						...
						<table class="table table-hover">
							<thead>
								<tr>
									...
									<th class="span5">
										<a id="color_code" href="javascript:void(0);" onclick="sortField('color_code');" style="color:#000;text-decoration: none;">
										Color
										<?php
												if($sort_field=='color_code')
												{
													if($sort_type=="DESC")
													{
							...
							<tbody>
								<?php
											...
											<td title="<?php echo $dataone->color_code;?>">
											<div style="background:<?php echo $dataone->color_code;?>; height: 20px; width: 50px;border: 2px solid;"></div></td>
											<td> <?php echo $dataone->color_code;?> </td>
...
<script type="text/javascript">
	$(function () {
        $('.Edituser').click(function (e) {
			...
			$("#frmValidation").attr("action", "<?php echo site_url('product_color/editcolor');?>");
	...
	$(function () {
        $('.Deleteuser').click(function (e) {
			...
			{
			    $('#ListingUserid').val($.trim(stripdata));
			    $("#frmValidation").attr("action", "<?php echo site_url('product_color/delete_category');?>");
			    $("#frmValidation").submit();
			    return true;
			}
		});
