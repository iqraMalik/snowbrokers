...
function go_reset()
{
	location.href="<?php echo base_url();?>index.php/event/index";
}
...
<div class="table-products">
	<div class="row-fluid head" style="border-bottom: 1px solid #C5C5C5; margin-bottom: 30px;">
		<div class="span12">
			<h4 class="management_name">Event Management</h4>
			<div class="pull-right" style="margin-bottom: 5px;"><a class="btn-flat new-product" href="<?php echo site_url('event/addevent'); ?>">+ Add New</a></div>
		</div>
	</div>
	<div class="row-fluid">
		<div id="main-stats">
			<div class="table-products">
				...
				<div class="row-fluid filter-block">
					...
					<div class="pull-left">
						<form name="search_form" id="search_form" method="post" action="">
							...
							?>
							<input type="hidden" id="sort_type" name="sort_type" value="<?php echo (isset($sort_type)?$sort_type:get_cookie('event_type'));?>" />
							<input type="hidden" id="sort_field" name="sort_field" value="<?php echo (isset($sort_field)?$sort_field:get_cookie('event_field'));?>" />
							...
							<div class="ui-select">
								<select name="search_option" id="search_option">
									...
									<option value="event_name" <?php if(isset($search_option) && ($search_option=='event_name')){echo 'selected';}?>>Event Name</option>
								</select>
							</div>
				...
				<form name="multiple_form" id="multiple_form" method="post" action="<?php echo site_url('event/multiple_option_event'); ?>">
						...
						<table class="table table-hover">
							<thead>
								<tr>
									...
									<th class="span3">
										<a id="event_name" href="javascript:void(0);" onclick="sortField('event_name');" style="color:#000;text-decoration: none;">
										Event Name
										<?php
											if($sort_field=='event_name')
											{
												if($sort_type=="DESC")
												{
													echo "<img src='".base_url()."images/sort_desc.png' />";
									...
									<th class="span3">
										<a id="recurring_type" href="javascript:void(0);" onclick="sortField('recurring_type');" style="color:#000;text-decoration: none;">
										Recurring Type
										<?php
											if($sort_field=='recurring_type')
											{
												if($sort_type=="DESC")
												{
													echo "<img src='".base_url()."images/sort_desc.png' />";
									...
									<th class="span3">
										<a id="recurring_upto" href="javascript:void(0);" onclick="sortField('recurring_upto');" style="color:#000;text-decoration: none;">
										Recurring Upto
										<?php
											if($sort_field=='recurring_upto')
											{
												if($sort_type=="DESC")
												{
													echo "<img src='".base_url()."images/sort_desc.png' />";
									...
									<th class="span3">
									<a id="start_date" href="javascript:void(0);" onclick="sortField('start_date');" style="color:#000;text-decoration: none;">
									Event Start Date
									<?php
									...
									<th class="span3">
									<a id="end_date" href="javascript:void(0);" onclick="sortField('end_date');" style="color:#000;text-decoration: none;">
								...
								<?php
									...
									?>
										...
										<tr class="first">
											...
											<?php $geteventcreator = $this->modelevent->Geteventcreatorsingle($dataone->user_id);
											...
											<td><?php echo $dataone->event_name; ?></td>
											<td><?php
											if($dataone->recurring_type==1)
											{
												echo "Weekly";	
											}
											elseif($dataone->recurring_type==2){
												echo "Monthly";
											}
											elseif($dataone->recurring_type==3){
												echo "Yearly";
											}
											else
											{
											...
											<td><?php echo $dataone->recurring_upto; ?></td>
											<td><?php echo $dataone->start_date ; ?></td>											
											<td><?php echo $dataone->end_date; ?></td>
											<td><?php if($dataone->status==1)
											{ echo "Active";}else{ echo "Blocked";} ?></td>
											<td>
												<ul class="actions">
...
<script type="text/javascript">
	$(function () {
        $('.Edituser').click(function (e) {
			...
			$("#frmValidation").attr("action", "<?php echo site_url('event/editevent');?>");
	...
	$(function () {
        $('.Deleteuser').click(function (e) {
			...
			{
			    $('#ListingUserid').val($.trim(stripdata));
			    $("#frmValidation").attr("action", "<?php echo site_url('event/delete_event');?>");
			    $("#frmValidation").submit();
			    return true;
			}
		});
